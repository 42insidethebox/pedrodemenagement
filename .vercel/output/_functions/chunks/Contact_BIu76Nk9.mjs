...
const $$Form = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<form> ${inputs && inputs.map((field) => {
    ...
    return renderTemplate`<div class="mb-6"> ${label && renderTemplate`<label${addAttribute(name, "for")} class="block text-sm font-medium"> ${label} </label>`} ${type === "select" ? renderTemplate`<select${addAttribute(name, "name")}${addAttribute(name, "id")} class="py-3 px-4 block w-full text-md rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-slate-900"> ${(options.length ? options : [{ value: "", label: "Select an option" }]).map((option) => renderTemplate`<option${addAttribute(option.value, "value")}> ${option.label ?? option.value} </option>`)} </select>` : renderTemplate`<input${addAttribute(type, "type")}${addAttribute(name, "name")}${addAttribute(name, "id")}${addAttribute(autocomplete, "autocomplete")}${addAttribute(placeholder, "placeholder")} class="py-3 px-4 block w-full text-md rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-slate-900">`} </div>`;
...
}, "/Users/pedroribeiro/iCloud Drive (Archive)/Documents/Coding/JS/monwebsite_factory/tonwebsite_ch/tonwebsite/src/components/ui/Form.astro", void 0);
...
}, "/Users/pedroribeiro/iCloud Drive (Archive)/Documents/Coding/JS/monwebsite_factory/tonwebsite_ch/tonwebsite/src/components/widgets/Contact.astro", void 0);
