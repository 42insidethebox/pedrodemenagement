...
const $$Tasks = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const loadTasks = async () => {
    try {
      ...
      console.error(error);
      if (tableBody) {
        tableBody.innerHTML = '<tr><td colspan="7" class="px-4 py-6 text-center text-sm text-red-400">Unable to load tasks.</td></tr>';
      }
    }
  };
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (message) {
          ...
          message.textContent = error instanceof Error ? error.message : 'Failed to save task.';
        }
      } finally {
        if (submitButton) {
