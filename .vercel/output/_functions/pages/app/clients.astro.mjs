...
const $$Clients = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const loadClients = async () => {
    try {
      ...
      console.error(error);
      if (tableBody) {
        tableBody.innerHTML = '<tr><td colspan="6" class="px-4 py-6 text-center text-sm text-red-400">Unable to load clients. Check your Supabase configuration.</td></tr>';
      }
    }
  };
  ...
  if (form) {
    form.addEventListener('submit', async (event) => {
      ...
      const payload = {
        ...
        primary_contact: String(formData.get('primary_contact') ?? '').trim(),
      ...
      try {
        ...
        if (message) {
          ...
          message.textContent = error instanceof Error ? error.message : 'Failed to save client.';
        }
      } finally {
        if (submitButton) {
  ...
  if (tableBody) {
    tableBody.addEventListener('click', async (event) => {
      ...
      try {
        await authFetch(\`/clients/\${deleteId}\`, {
          method: 'DELETE',
          parseJson: false,
        });
        await loadClients();
