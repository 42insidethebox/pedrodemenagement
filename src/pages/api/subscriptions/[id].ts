...
export const DELETE: APIRoute = withAuth(async ({ params }) => {
  ...
  try {
    ...
    await updateOrderStatusBySubscriptionId(subscriptionId, 'cancelled', {
...
export const PATCH: APIRoute = withAuth(async ({ params, request }) => {
  const subscriptionId = params.id;
  if (!subscriptionId) {
    return new Response(JSON.stringify({ error: 'Missing subscription id' }), { status: 400 });
  }
  ...
  try {
    ...
    await updateOrderStatusBySubscriptionId(subscriptionId, 'active', {
