...
export const GET: APIRoute = withAuth(async ({ locals, params }) => {
  ...
  try {
    ...
    const order = await getOrderById(client, agency.id, orderId);
...
export const PATCH: APIRoute = withAuth(async ({ locals, params, request }) => {
  ...
  try {
    const { agency, client } = await getAgencyContext(locals);
    const existing = await getOrderById(client, agency.id, orderId);
    const updated = await updateOrder(client, agency.id, orderId, payload);
