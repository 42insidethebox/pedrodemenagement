...
import { parseDocumentPayload } from '~/utils/backend/validation';
...
export const GET: APIRoute = withAuth(async ({ locals }) => {
  try {
    ...
    if (error) {
      ...
      return new Response(JSON.stringify({ error: 'Unable to load documents' }), { status: 500 });
    }
    ...
    console.error('Unexpected error in GET /api/backend/documents', error);
...
export const POST: APIRoute = withAuth(async ({ locals, request }) => {
  ...
  try {
    ...
    if (error) {
      ...
      return new Response(JSON.stringify({ error: 'Unable to save document' }), { status: 500 });
    }

    await logAgencyActivity(client, agency.id, 'document_created', 'document', data.id, {
      title: data.title,
    ...
    console.error('Unexpected error in POST /api/backend/documents', error);
    return new Response(JSON.stringify({ error: 'Unexpected server error' }), { status: 500 });
  }
});
