...
export function getBrandFromHost(host: string | null | undefined): BrandKey {
  ...
  if (value.includes('onglesgel.ch')) return 'onglesgel';
  ...
  if (value.includes('precisionsystems.ch')) return 'precisionsystems';
  if (value.includes('iopartner.ch')) return 'iopartner';
